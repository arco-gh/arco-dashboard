...
if vista == "🏢  Panel Operativo":
    ...
    fecha_corte = dm["fecha"].max()   # último mes con dato real
    fecha_pred  = pd.Timestamp(year=año_sel, month=mes_sel, day=1)

    fig2, axes = plt.subplots(1, 2, figsize=(15, 4.5))

    # ── Panel izq: Afluencia histórica + proyección acumulada ────────────────
    ax_a = axes[0]

    # Línea sólida: datos reales 2023–2025
    ax_a.plot(dm["fecha"], dm["afluencia_mensual"],
              color=C_SEC, lw=2.2, marker="o", ms=4,
              label="Afluencia real (2023–2025)", zorder=5)

    # Banda IC 95% de Prophet (solo si viene de Prophet)
    if df_proy["fuente"].iloc[0] == "Prophet + eventos":
        ax_a.fill_between(
            df_proy["fecha"],
            df_proy["afluencia_lower"],
            df_proy["afluencia_upper"],
            alpha=0.15, color=C_NARJ,
            label="IC 95% Prophet"
        )

    # Línea punteada: proyección acumulada enero → mes seleccionado
    ax_a.plot(df_proy["fecha"], df_proy["afluencia_pred"],
              color=C_NARJ, lw=2, ls="--", marker="o", ms=5,
              label=f"Proyección {año_sel} (ene – {mes_nombre})", zorder=4)

    # Resaltar el mes seleccionado específicamente
    ax_a.scatter(
        [fecha_pred], [pred["afluencia_pred"]],
        color=C_POS, s=140, zorder=6, edgecolors="white", lw=1.5,
        label=f"{mes_nombre} {año_sel}: {pred['afluencia_pred']:,.0f}"
    )

    # Separador histórico / proyección
    ax_a.axvline(fecha_corte, color=C_GRIS, lw=1.2, ls=":", alpha=0.8)
    ax_a.text(fecha_corte, ax_a.get_ylim()[1] if ax_a.get_ylim()[1] > 0 else 400000,
              "  ← histórico | proyección →",
              fontsize=7.5, color=C_GRIS, va="top")

    ax_a.set_title(f"Afluencia mensual — histórico y proyección {año_sel}",
                   fontsize=10.5, fontweight="600", color=C_PRIM)
    ax_a.set_ylabel("Visitantes", fontsize=9)
    ax_a.xaxis.set_major_formatter(mdates.DateFormatter("%b %y"))
    plt.setp(ax_a.xaxis.get_majorticklabels(), rotation=35, ha="right")
    ax_a.yaxis.set_major_formatter(
        mticker.FuncFormatter(lambda v, _: f"{v/1e3:.0f}K"))
    ax_a.legend(fontsize=8, loc="upper left")

    # ── Panel der: Ratio gasto/ingresos histórico + proyección acumulada ─────
    ax_b = axes[1]

    # Barras históricas coloreadas por nivel de ratio
    colores_ratio = [C_POS if r <= 0.20 else C_NARJ if r <= 0.30 else C_ACENT
                     for r in dm["ratio_gasto_ingresos"]]
    ax_b.bar(dm["fecha"], dm["ratio_gasto_ingresos"] * 100,
             color=colores_ratio, alpha=0.70, width=20,
             label="Ratio real (2023–2025)")

    # Banda IC 95% del ratio (propagada desde la banda de afluencia de Prophet)
    if df_proy["fuente"].iloc[0] == "Prophet + eventos":
        ing_p = dm["ingresos_por_renta"].mean()
        # El ratio también varía con la banda de afluencia a través del Modelo B
        # Aproximación conservadora: usar ±la misma proporción que la banda de afluencia
        ratio_proy   = df_proy["ratio_pred"].values * 100
        af_rango_pct = ((df_proy["afluencia_upper"] - df_proy["afluencia_lower"])
                        / (2 * df_proy["afluencia_pred"])).values
        ratio_lower  = ratio_proy * (1 - af_rango_pct * 0.5)
        ratio_upper  = ratio_proy * (1 + af_rango_pct * 0.5)
        ax_b.fill_between(
            df_proy["fecha"],
            ratio_lower, ratio_upper,
            alpha=0.15, color=C_NARJ, label="IC 95% aprox."
        )

    # Línea punteada: ratio proyectado acumulado
    ax_b.plot(df_proy["fecha"], df_proy["ratio_pred"] * 100,
              color=C_NARJ, lw=2, ls="--", marker="o", ms=5,
              label=f"Proyección ratio {año_sel}", zorder=4)

    # Resaltar mes seleccionado
    ax_b.scatter(
        [fecha_pred], [pred["ratio_pred"] * 100],
        color=C_POS, s=140, zorder=6, edgecolors="white", lw=1.5,
        label=f"{mes_nombre} {año_sel}: {pred['ratio_pred']:.1%}"
    )

    # Líneas de referencia
    ax_b.axhline(10, color=C_POS, ls="-.", lw=2, label="Meta 10%")
    ax_b.axhline(dm["ratio_gasto_ingresos"].mean() * 100,
                 color=C_GRIS, ls=":", lw=1.3,
                 label=f"Prom. histórico {dm['ratio_gasto_ingresos'].mean():.1%}")

    # Separador
    ax_b.axvline(fecha_corte, color=C_GRIS, lw=1.2, ls=":", alpha=0.8)

    ax_b.set_title(f"Ratio gasto / ingresos — histórico y proyección {año_sel}",
                   fontsize=10.5, fontweight="600", color=C_PRIM)
    ax_b.set_ylabel("%", fontsize=9)
    ax_b.xaxis.set_major_formatter(mdates.DateFormatter("%b %y"))
    plt.setp(ax_b.xaxis.get_majorticklabels(), rotation=35, ha="right")
