...
if vista == "🏢  Panel Operativo":
    ...
    st.markdown("""
    <div class="section-card">
    <div class="section-title">📈 Tendencia histórica de afluencia (2023–2025)</div>
    """, unsafe_allow_html=True)

    fig2, axes = plt.subplots(1, 2, figsize=(14, 4))

    # Panel izq: Afluencia real vs predicha
    ax_a = axes[0]
    ax_a.plot(dm["fecha"], dm["afluencia_mensual"],
              color=C_SEC, lw=2, marker="o", ms=4,
              label="Afluencia real")
    # Línea de predicción histórica
    X_af_hist = dm[modelos["feat_af"]]
    y_pred_hist = modelos["mod_af"].predict(X_af_hist)
    ax_a.plot(dm["fecha"], y_pred_hist,
              color=C_NARJ, lw=1.8, ls="--", alpha=0.8,
              label="Modelo (ajustado)")
    # Punto de predicción futura
    fecha_pred = pd.Timestamp(year=año_sel, month=mes_sel, day=1)
    ax_a.scatter([fecha_pred], [pred["afluencia_pred"]],
                 color=C_POS, s=120, zorder=6,
                 label=f"Predicción {mes_nombre} {año_sel}")
    ax_a.axvline(dm["fecha"].max(), color=C_GRIS, lw=1, ls=":", alpha=0.7)
    ax_a.set_title("Afluencia mensual", fontsize=10.5,
    ...
    # Panel der: Ratio gasto/ingresos histórico
    ax_b = axes[1]
    ...
    ax_b.bar(dm["fecha"], dm["ratio_gasto_ingresos"] * 100,
             color=colores_ratio, alpha=0.75, width=20)
    ax_b.axhline(10, color=C_POS, ls="-.", lw=2, label="Meta 10%")
    ax_b.axhline(dm["ratio_gasto_ingresos"].mean() * 100,
                 color=C_GRIS, ls=":", lw=1.5,
                 label=f"Promedio {dm['ratio_gasto_ingresos'].mean():.1%}")
    # Punto de predicción
    ax_b.scatter([fecha_pred], [pred["ratio_pred"] * 100],
                 color=C_POS, s=120, zorder=6,
                 label=f"Pred. {mes_nombre}: {pred['ratio_pred']:.1%}")
    ax_b.set_title("Ratio gasto / ingresos (%)", fontsize=10.5,
                   fontweight="600", color=C_PRIM)
